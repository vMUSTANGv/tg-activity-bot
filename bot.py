...
async def cmd_summary(msg: Message):
    ...
    system = (
        "Ты — секретарь чата встреч по интересам. Твоя единственная задача: вытащить из лога "
        "только организационную информацию — встречи, договорённости, предложения, важные ссылки. "
        "Игнорируй мемы, шутки, эмоции, болтовню, спорах ни о чём, общие реакции. "
        "Если в логе нет конкретики по разделу — пиши «—», не выдумывай. "
        "Никаких имён и фамилий, только @username из лога. "
        "Отвечай на русском, кратко, по делу, в HTML."
    )
    user_prompt = (
        f"Лог последних {len(rows)} сообщений (участники как @username):\n\n{chat_log}\n\n"
        "Сделай повестку в этом формате (если раздел пустой — пиши «—»):\n\n"
        "📅 <b>Встречи</b>\n"
        "• <b>Подтверждённые</b>: дата/время/место/кто идёт — только если реально согласовано\n"
        "• <b>Предложенные</b>: кто предложил, что именно, ждут ли реакции\n"
        "• <b>Прошедшие</b>: упоминания состоявшихся встреч с краткой оценкой если есть\n\n"
        "✅ <b>Договорённости</b>\n"
        "Только конкретные решения: «договорились о X на дату Y», «решили купить Z», «определили цену N». Каждая строка — одна договорённость.\n\n"
        "💡 <b>Идеи и предложения</b>\n"
        "Кто что предложил по теме встреч/активностей, на что ещё нет реакции. Без болтовни.\n\n"
        "🔗 <b>Важные ссылки</b>\n"
        "Только реально полезные ресурсы (мероприятия, карты, статьи). Без картинок и мемов.\n\n"
        "⚠️ <b>Открытые вопросы</b>\n"
        "О чём обсуждали но не договорились — нужно решить.\n\n"
        "Жёсткие правила:\n"
        "— Никакого «обсуждали разное», «делились мнениями», «обменивались шутками».\n"
        "— Если организационной инфы нет вообще — все разделы «—», добавь одну строку: «🤷 За эти {len(rows)} сообщений организационных тем не было».\n"
        "— Максимум 350 слов.\n"
        "— Только @username."
    )
    try:
